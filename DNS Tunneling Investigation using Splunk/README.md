...
timeline
    ...
    T+0s : Phishing email delivered to Outlook
    T+5s : User opens attachment
    T+10s : User enables macros : WINWORD.EXE launched : Sysmon Event ID 1
    T+15s : Malicious macro executes : Sysmon Event ID 1
    T+20s : Scheduled Task created : Persistence established : Sysmon Event ID 11
    T+3min : First DNS query to gOogle.com : Sysmon Event ID 22
    T+6min : Second DNS query to gOogle.com : Sysmon Event ID 22
    T+9min : Third DNS query to gOogle.com : Sysmon Event ID 22
    Continuous : DNS tunneling repeats every 3 minutes : Sysmon Event ID 22
```
